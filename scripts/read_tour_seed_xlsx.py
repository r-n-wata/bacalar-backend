...
def find_sheet_path(archive: zipfile.ZipFile, target_names: list[str]) -> str:
    ...
    for sheet in workbook.findall("main:sheets/main:sheet", NS):
        if sheet.attrib.get("name") not in target_names:
            continue
    ...
    raise SystemExit(
        f'Sheets {", ".join(target_names)} not found in workbook.'
    )
...
def load_rows(workbook_path: Path) -> list[dict[str, str | None]]:
    with zipfile.ZipFile(workbook_path) as archive:
        ...
        sheet_path = find_sheet_path(archive, ["Tours", "Experiences"])
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: read_tour_seed_xlsx.py <workbook-path>")
